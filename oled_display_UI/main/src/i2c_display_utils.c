...
const uint8_t bluetooth_icon[2][7] = {
    {0x84, 0x48, 0x30, 0xFF, 0x31, 0x4A, 0x84},
...
esp_err_t send_command(i2c_master_dev_handle_t dev_handle, uint8_t command) {
    uint8_t write_buf[2] = { COMMAND_BYTE, command };
...
void clear_screen(i2c_master_dev_handle_t dev_handle) {
    uint8_t pixel_buf[128] = { 0 };
    for (int page = 0; page < 8; ++page) {
        set_cursor(dev_handle, 2, page);
        draw(dev_handle, pixel_buf, sizeof(pixel_buf));
    }
}


void display_init(i2c_master_dev_handle_t dev_handle) {
    send_command(dev_handle, SET_DISPLAY_ON);
    send_command(dev_handle, NORMAL_MODE);
    send_command(dev_handle, PAGE_ADDRESSING_MODE);       // Set addressing mode to page addressing
